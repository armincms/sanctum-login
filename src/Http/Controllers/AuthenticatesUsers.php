...
{  
    ...
    {
        return with(User::where($this->credentials($request))->first(), function($user) use ($request) { 
        	return app('hash')->check($request->password, optional($user)->password) ? $user : null;
        });
    }
